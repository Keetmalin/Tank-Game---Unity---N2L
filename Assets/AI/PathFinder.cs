...
{
    /// <summary>
    /// Finds the shortest path to a given Weight
    /// </summary>
    class PathFinder
    {
        Map ActorMap;
        ...
        private void generateMap()
        {
            for (int i = 0; i < width; i++)
            {
                ...
                {
                    GameObject actor = ActorMap.getGameObject(i, j);
        ...
        public Stack<Weight> generateShortestPath(int x, int y)
        {
            openList.Clear();
            closedList.Clear();
            ...
            {
                case AI.AIMode.Defensive:
                    NodeType.Lifepack = 2;
                    NodeType.Coins = 5;
                    NodeType.Tank = 5;
                    break;
                case AI.AIMode.Greedy:
                    NodeType.Coins = 2;
                    NodeType.Lifepack = 5;
                    NodeType.Tank = 5;
                    break;
                case AI.AIMode.Offensive:
                    NodeType.Lifepack = 4;
                    NodeType.Coins = 5;
                    NodeType.Tank = 2;
                    break;
            ...
            //setting the start Weight for the calculation
            setStartNode();
        ...
        /// </summary>
        /// <returns>A List of Weights containing the path</returns>
        private Stack<Weight> findPath()
        {
            Stack<Weight> path = new Stack<Weight>();
            ...
            {
                Weight weight = new Weight(current.X, current.Y);
                weight.FirstDirection = current.DirectionAtNode;
                weight.LastDirection = current.LastDirectionAtNode;
                path.Push(weight);// adds the coordinates of the current node
                current = current.Parent;// set the next check to its parent
            }
            this.path = path;
            System.Console.WriteLine("Target : " + endX + "," + endY + " ");
            foreach (Weight weight in path.ToArray())
            {
                System.Console.Write("<" + weight.X + " " + weight.Y + ">");
            }
        ...
        {
            pathLength = 0;
            Weight[] weights = path.ToArray();
            foreach (Weight weight in weights)
            {
                pathLength += map[weight.X, weight.Y].F;
            }
            return pathLength;
        }
        ...
        internal Stack<AICell> CalculateShortestPath(AICell target, AICell current)
        {
            ...
            Stack<Weight> pathWeights = generateShortestPath(endX, endY);
            Stack<AICell> pathCells = new Stack<AICell>();
            foreach (Weight weight in pathWeights)
            {
